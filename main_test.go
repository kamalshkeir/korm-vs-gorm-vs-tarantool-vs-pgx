...
const kormCockroachConnStr = `cockroach:password@localhost:26257`
...
func TestMain(m *testing.M) {
	...
	//korm.SetCacheMaxMemory(1) // to make realistic benchmark, case when database can be 10x larger than RAM (not realistic anymore)

	//korm.DisableCache()
	// ^ the cheat for 2m performance (not allowed to disable cache anymore)
	// caveat:
	// - database cannot be more than half of RAM size, since the cache has no limit
	// - you cannot update record using another tool (eg. database IDEs) since it didn't evict the cache (cache is flushed every 10 minutes, so even if someone would do that, wait 10 minutes or execute korm.FlushCache manualy programaticaly)
